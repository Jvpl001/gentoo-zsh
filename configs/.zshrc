...
zsh-system-compile() {
  ...
  # Enable extended globbing locally just for this function
  setopt localoptions extendedglob
  
  # ^*.zwc means "exclude .zwc files"
  # (N.) applies NullGlob and ensures we only match regular files (ignoring directories)
  for file in /usr/share/zsh/site-functions/^*.zwc(N.); do
    # Only compile if the .zwc doesn't exist, or if the text file is newer
    if [[ ! -f "${file}.zwc" || "$file" -nt "${file}.zwc" ]]; then
      # Run zcompile inside a root Zsh process
      doas zsh -c "zcompile '$file'"
    fi
  done
  
  # --- NEW: Detect and clean up orphaned .zwc files ---
  local orphan_zwc=()
  for zwc_file in /usr/share/zsh/site-functions/*.zwc(N.); do
    # Strip the .zwc extension to get the expected source file name
    local source_file="${zwc_file%.zwc}"
    
    # If the uncompiled completion file is gone, the .zwc is an orphan
    if [[ ! -f "$source_file" ]]; then
      orphan_zwc+=("$zwc_file")
    fi
  done

  if (( ${#orphan_zwc[@]} > 0 )); then
    echo "\nFound ${#orphan_zwc[@]} orphaned system completion bytecode file(s):"
    for file in "${orphan_zwc[@]}"; do
      echo "  - $file"
    done
    echo ""
    
    if read -q "reply?Do you want to delete them? (requires doas) [y/N] "; then
      echo "" 
      # Execute deletion as root
      doas rm -f "${orphan_zwc[@]}"
...
alias rm='rm -vI'                      # Prompt ONCE before deleting 3+ files (less annoying than -i)
